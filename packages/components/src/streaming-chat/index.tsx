  ...
  return (
    <div className={cn("flex flex-col h-[400px] rounded-2xl border border-white/10 bg-[#0d0f16]", className)}>
      <div className="flex-1 overflow-y-auto space-y-3 p-4 b-scroll" role="log" aria-live="polite" aria-label="Chat messages">
        {messages.map((m) => (
          <div key={m.id} className={cn("max-w-[85%] rounded-xl px-3 py-2 text-sm", m.role === "user" ? "ml-auto bg-[--b-accent] text-[--b-accent-foreground]" : "bg-white/[0.06] text-white/90")}>
            <p>{m.text}</p>
            {m.citations && (
        ...
        {(streaming || text) && (
          <div className="max-w-[85%] rounded-xl bg-white/[0.06] px-3 py-2 text-sm text-white/90">
            <p>{text}<span className="ml-0.5 inline-block h-4 w-[2px] animate-pulse bg-[--b-accent] align-middle" aria-hidden /></p>
          </div>
        )}
      </div>
      <form className="flex gap-2 border-t border-white/10 p-3" onSubmit={(e) => { e.preventDefault(); if (!input.trim()) return; setMessages((m) => [...m, { id: `u-${Date.now()}`, role: "user", text: input }]); setInput(""); stream(); }}>
        ...
        <input id="chat-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about components… (demo only)" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm outline-none focus:border-[--b-accent]" />
        <button className="rounded-xl bg-[--b-accent] px-4 py-2 text-sm font-bold text-[--b-accent-foreground] disabled:opacity-50" disabled={streaming}>Send</button>
      </form>
...
export function ModelSelector({ models = ["buildora-1-pro", "buildora-1-flash", "oss-70b"], className }: { models?: string[]; className?: string }) {
  ...
  return (
    <label className={cn("inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-1.5 text-sm", className)}>
      ...
      <span className="h-2 w-2 rounded-full bg-[--b-success]" aria-hidden />
