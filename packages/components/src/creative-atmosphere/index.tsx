...
export function CursorSpotlight({ className, children }: { className?: string; children?: React.ReactNode }) {
  ...
  return (
    <div ref={ref} onPointerMove={onMove} className={cn("group relative overflow-hidden rounded-2xl border border-[color-mix(in_oklab,var(--b-border)_10%,transparent)] bg-[color-mix(in_oklab,var(--b-text)_2%,transparent)] p-6", className)}>
...
export function TactileLoader({ label = "Loading", className }: { label?: string; className?: string }) {
  return (
    <div className={cn("inline-flex items-center gap-2", className)} role="status" aria-label={label}>
      {[0, 1, 2].map((i) => (
        <span key={i} className="h-2 w-2 rounded-full bg-[color:var(--b-accent)] animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
      ))}
